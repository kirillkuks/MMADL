...
class MMADLTranslatorTexVisitor(MMADLTranslatorVisitor):
    ...
    def visitColon(self, ctx: MMADLParser.ColonContext): self.addToken(self.code_printer.printCodeText(ctx.COLON()))

    def visitComma(self, ctx: MMADLParser.CommaContext): self.addToken(self.code_printer.printCodeText(ctx.COMMA()))
    ...
    def visitOutput_params(self, ctx: MMADLParser.Output_paramsContext):
        for i in range(len(ctx.param_type())):
            self.visit(ctx.param_type(i))
            if ctx.comma() is not None and ctx.comma(i) is not None:
                self.visit(ctx.comma(i))
